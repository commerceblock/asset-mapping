...
#The zero ratio is the token ratio at time zero
    init_ratio = 0.1
#calculate the number of 8-hour periods based on the blockheight
    hours = blockheight // 480
#calculate the token ratio iteratively based on intermediate rounding to 6 deciaml places
    total = round(mass/init_ratio,8)
    for it in range(hours):
        reissue = round(total*(1.0+float(INFLATION_RATE))**(1.0/(3*365))-total,8)
        total += reissue
...
#function to save json object to file
    ...
    def export_json(self,filename,append=False):
#function to save json object to file
        if append:
